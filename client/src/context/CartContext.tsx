...
export const CartProvider: React.FC<{ children: React.ReactNode, username: string, token: string }> = ({ children, username, token }) => {
    ...
    const addProductToCart = async (productId: string, name: string, price: number, amount: number, img: string) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_BASE_URL}/cart/add.php`, `username=${username}&token=${token}&pid=${productId}&amount=${amount}`, {
    ...
    const updateProductAmount = async (productId: string, newAmount: number) => {
        ...
        saveCartItems(updatedCartItems);
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/cart/update.php`, `username=${username}&token=${token}&pid=${productId}&amount=${newAmount}`, {
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            });
    ...
    const checkoutCart = async (address: string, phone: string) => {
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/cart/purchase.php`, `username=${username}&token=${token}&address=${encodeURIComponent(address)}&phone=${encodeURIComponent(phone)}&ship=${shippingCost}`, {
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            });
            if (response.status === 200) {
